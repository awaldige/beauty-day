...
export function Agenda({ agendamentos = [], profissionais = [], servicos = [] }: AgendaProps) {
  ...
  return (
    <div className="space-y-6 md:space-y-10 px-2 sm:px-4 md:px-0">
      ...
      <div className="relative overflow-hidden bg-white/80 backdrop-blur-md rounded-2xl md:rounded-3xl border border-violet-200/80 shadow-[0_10px_30px_rgba(109,40,217,0.05)]">
        <div className="absolute top-0 left-0 right-0 h-[4px] bg-gradient-to-r from-violet-600 via-purple-500 to-fuchsia-600" />
        
        <div className="px-5 py-4 md:px-8 md:py-6 border-b border-violet-200/50 flex items-center justify-between bg-gradient-to-r from-violet-50/50 to-fuchsia-50/30">
          <div>
            <h2 className="text-sm md:text-base font-black tracking-tight text-[#1a0933] flex items-center gap-2">
              <span>📅</span> Novo Atendimento
            </h2>
            <p className="text-[11px] md:text-xs text-violet-800/80 font-semibold mt-0.5">
              Selecione o profissional, serviço e horário desejados.
            </p>
          </div>
        </div>

        <form onSubmit={handleSubmeter} className="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6 items-end">
          ...
          <div className="space-y-1.5">
            ...
            <select 
              ...
              required
              style={selectStyle}
              className="w-full h-12 md:h-14 px-3.5 md:px-4 text-xs md:text-sm font-bold border-2 border-violet-100 rounded-xl md:rounded-2xl bg-white text-[#1a0933] focus:outline-none focus:border-violet-500 transition-all cursor-pointer appearance-none"
            >
          ...
          <div className="space-y-1.5">
            ...
            <select 
              ...
              style={selectStyle}
              className="w-full h-12 md:h-14 px-3.5 md:px-4 text-xs md:text-sm font-bold border-2 border-violet-100 rounded-xl md:rounded-2xl bg-white text-[#1a0933] focus:outline-none focus:border-violet-500 transition-all cursor-pointer appearance-none disabled:bg-gray-100/80 disabled:cursor-not-allowed disabled:opacity-60"
            >
              <option value="">
          ...
          <div className="space-y-1.5">
            ...
            <input 
              ...
              required
              className={`w-full h-12 md:h-14 px-3.5 md:px-4 text-xs md:text-sm font-bold border-2 rounded-xl md:rounded-2xl bg-white text-[#1a0933] focus:outline-none transition-all cursor-pointer appearance-none ${
                conflitoAgendamento 
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative flex items-center rounded-xl md:rounded-2xl border-2 border-violet-100 bg-white focus-within:border-violet-500 transition-all overflow-hidden">
              <div className="flex items-center justify-center bg-violet-50 text-violet-700 font-black text-xs px-3.5 md:px-5 h-12 md:h-14 border-r-2 border-violet-100 select-none shrink-0">
                R$
              </div>
              <input 
                type="number" 
                ...
                required 
                placeholder="0,00"
                className="w-full h-12 md:h-14 px-3 bg-transparent text-xs md:text-sm font-black text-[#1a0933] focus:outline-none appearance-none antialiased"
              />
            </div>
          </div>
          ...
          <div className="sm:col-span-2 lg:col-span-1 pt-2 lg:pt-0">
            <button 
              ...
              className={`w-full h-12 md:h-14 text-xs font-black rounded-xl md:rounded-2xl shadow-lg transition-all active:scale-[0.98] uppercase tracking-widest flex items-center justify-center gap-2 ${
                conflitoAgendamento
        ...
        {conflitoAgendamento && (
          <div className="px-4 pb-4 md:px-8 md:pb-6 -mt-1">
            <div className="p-3 md:p-4 bg-rose-100/80 border border-rose-300 rounded-xl md:rounded-2xl flex items-center gap-3 text-rose-900 text-xs font-bold shadow-sm">
              <span className="text-base shrink-0">🚨</span>
              <div>
                <strong>Conflito:</strong> {conflitoAgendamento.profissional_nome} já possui atendimento marcado neste horário.
              </div>
            </div>
          </div>
        )}
      ...
      {/* 2. PAINEL DE CONTROLE DE BUSCA E FILTROS */}
      <div className="bg-white/80 backdrop-blur-md p-4 md:p-6 rounded-2xl md:rounded-3xl border border-violet-200/70 shadow-[0_10px_30px_rgba(109,40,217,0.03)] space-y-4">
        
        {/* Filtros Rápidos com Rolagem Suave no Mobile */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-violet-100 pb-4">
          <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-none -mx-2 px-2 sm:mx-0 sm:px-0">
            <span className="text-[10px] font-black uppercase text-violet-900 tracking-wider shrink-0 mr-1">Visualização:</span>
            
            <button
              type="button"
              onClick={() => { setIntervaloData('hoje'); setFiltroDataEspecifica(''); setPaginaAtual(1); }}
              className={`h-9 px-3.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shrink-0 active:scale-95 ${
          ...
          <div className="text-xs font-bold text-violet-900 bg-violet-100/60 px-3 py-1.5 rounded-xl self-start sm:self-auto">
            Total: <strong className="text-violet-950 font-black">{agendamentosFiltrados.length}</strong>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="relative">
            <input 
              type="text" 
              placeholder="Buscar profissional, serviço..." 
              value={filtroPesquisa}
              onChange={(e) => { setFiltroPesquisa(e.target.value); setPaginaAtual(1); }}
              className="w-full h-11 px-4 pl-9 text-xs border-2 border-violet-100 rounded-xl bg-white text-[#1a0933] placeholder-violet-300 font-bold focus:outline-none focus:border-violet-400 transition-all"
            />
          ...
          <select
            ...
            style={selectStyle}
            className="h-11 px-4 pr-9 text-xs font-bold border-2 border-violet-100 rounded-xl bg-white text-[#1a0933] focus:outline-none focus:border-violet-400 transition-all cursor-pointer appearance-none"
          >
          ...
          <input 
            ...
            className="h-11 px-4 text-xs font-bold border-2 border-violet-100 rounded-xl bg-white text-[#1a0933] focus:outline-none focus:border-violet-400 transition-all cursor-pointer"
          />
        </div>
        ...
        {(filtroPesquisa || filtroProfissional || filtroStatus !== 'todos' || filtroDataEspecífica || intervaloData !== 'hoje') && (
          <div className="flex justify-end pt-1">
            ...
            >
              <span>✨</span> Resetar Filtros
            </button>
          </div>
        )}
      </div>

      {/* 3. LISTAGEM ADAPTÁVEL: CARDS NO MOBILE / TABELA NO DESKTOP */}
      <div className="bg-white/70 backdrop-blur-md rounded-2xl md:rounded-3xl border border-violet-200/60 shadow-[0_20px_50px_rgba(109,40,217,0.04)] overflow-hidden">
        
        {agendamentosPaginados.length === 0 ? (
          <div className="p-8 text-center text-violet-800/70 font-bold text-xs md:text-sm">
            🔍 Nenhum agendamento encontrado para o filtro selecionado.
          </div>
        ) : (
          <>
            {/* --- VISUALIZAÇÃO EM CARDS (MOBILE: esconde no md) --- */}
            <div className="block md:hidden divide-y divide-violet-100">
              {agendamentosPaginados.map((agend) => (
                <div 
                  key={agend.id}
                  className={`p-4 space-y-3 ${
                    agend.status === 'cancelado' ? 'opacity-60 bg-rose-50/20' : 
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="font-black text-xs text-[#1a0933]">{agend.profissional_nome}</p>
                        <p className="text-[10px] font-bold text-violet-700 uppercase">{agend.servico_nome}</p>
                      </div>
                    </div>

                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-[9px] font-black uppercase tracking-wider border ${
                      agend.status === 'concluido' ? 'bg-sky-50 text-sky-800 border-sky-300/40' :
                      agend.status === 'confirmado' ? 'bg-emerald-50 text-emerald-800 border-emerald-300/40' :
                      agend.status === 'cancelado' ? 'bg-rose-50 text-rose-800 border-rose-300/40' :
                      ...
                      <span className={`h-1.5 w-1.5 rounded-full ${
                        ...
                        agend.status === 'cancelado' ? 'bg-rose-500' : 'bg-amber-500'
                      }`} />
                      {agend.status}
                    </span>
                  </div>

                  <div className="flex items-center justify-between text-xs bg-violet-50/50 p-2.5 rounded-xl border border-violet-100/60">
                    <div className="font-mono text-violet-900 font-bold text-[11px]">
                      {reagendandoId === agend.id ? (
                        <input 
                          type="datetime-local" 
                          value={novaDataHora} 
                          onChange={(e) => setNovaDataHora(e.target.value)}
                          className="h-8 px-2 text-[11px] text-[#1a0933] bg-white border border-violet-300 rounded-lg focus:outline-none"
                        />
                      ) : (
                    ...
                    <div className="font-black text-[#1a0933]">
                      R$ {Number(agend.valor_cobrado || 0).toFixed(2)}
                    </div>
                  </div>

                  {/* Ações do Card no Mobile */}
                  <div className="flex items-center justify-end gap-1.5 pt-1">
                      ...
                      reagendandoId === agend.id ? (
                        <div className="flex gap-1.5 w-full justify-end">
                          <button 
                            ...
                            className="text-[10px] text-white font-black bg-emerald-600 px-3 py-2 rounded-xl active:scale-95 uppercase"
                          >
                            Salvar
                          </button>
                          <button 
                            ...
                            className="text-[10px] text-violet-800 font-black bg-violet-100 px-3 py-2 rounded-xl active:scale-95 uppercase"
                          >
                            Sair
                          </button>
                        </div>
                      ) : (
                        <div className="flex gap-1.5 flex-wrap justify-end w-full">
                          <button 
                            type="button"
                            onClick={() => setIdParaConcluir(agend.id)}
                            disabled={mutationConcluir.isPending}
                            className="text-[10px] text-emerald-900 font-black bg-emerald-100 hover:bg-emerald-200 px-3 py-2 rounded-xl border border-emerald-300/40 uppercase active:scale-95"
                          >
                            ✓ Concluir
                          </button>
                          ...
                          <button 
                            ...
                            className="text-[10px] text-violet-900 font-black bg-violet-200/60 hover:bg-violet-200 px-3 py-2 rounded-xl border border-violet-300/40 uppercase active:scale-95"
                          >
                            Reagendar
                          </button>
                          ...
                          <button 
                            ...
                            className="text-[10px] text-rose-800 font-black bg-rose-100 hover:bg-rose-200 px-3 py-2 rounded-xl border border-rose-300/40 uppercase active:scale-95"
                          >
                            Cancelar
                          </button>
                        </div>
                    ...
                    {agend.status === 'concluido' && (
                      <span className="text-[10px] font-bold text-emerald-700 bg-emerald-100/60 px-2.5 py-1 rounded-lg">
            ...
            {/* --- VISUALIZAÇÃO EM TABELA (DESKTOP: mostra a partir do md) --- */}
                    ...
                    >
                      ...
                      <td className="px-6 py-4.5 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          {agend.status === 'concluido' && (
                            <span className="text-[10px] font-bold text-emerald-700 bg-emerald-100/60 px-3 py-1.5 rounded-xl border border-emerald-200/50">
                              Finalizado
                            </span>
                          )}
                        </div>
        ...
        {/* Paginação Adaptável */}
        {totalPaginas > 1 && (
          <div className="px-4 py-3 md:px-6 md:py-4 bg-violet-50/40 border-t border-violet-100 flex items-center justify-between text-xs font-bold text-violet-900">
            <button 
              disabled={paginaAtual === 1}
              onClick={() => setPaginaAtual(p => p - 1)}
              className="px-3 py-1.5 rounded-lg bg-white border border-violet-200 disabled:opacity-40"
            >
              Anterior
            </button>
